...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	private static double skipRoad = VP.gap + Property.getProperty().getRoadLength();
	...
	public void addLight(Light d, int i, int j) {
		...
		Translator t = new TranslatorWE(x, y, Property.getProperty().getCarLength(), VP.elementWidth, VP.scaleFactor);
		painter.addLight(d,t);
	}
	public void addHorizontalRoad(Road l, int i, int j, boolean eastToWest) {
		double x = skipInit + j*skipRoadCar;
		double y = skipInit + skipRoad + i*skipRoadCar;
		Translator t = eastToWest ? new TranslatorEW(x, y, Property.getProperty().getRoadLength(), VP.elementWidth, VP.scaleFactor)
				: new TranslatorWE(x, y, Property.getProperty().getRoadLength(), VP.elementWidth, VP.scaleFactor);
		painter.addRoad(l,t);
	}
	public void addVerticalRoad(Road l, int i, int j, boolean southToNorth) {
		double x = skipInit + skipRoad + j*skipRoadCar;
		double y = skipInit + i*skipRoadCar;
		Translator t = southToNorth ? new TranslatorSN(x, y, Property.getProperty().getRoadLength(), VP.elementWidth, VP.scaleFactor)
				: new TranslatorNS(x, y, Property.getProperty().getRoadLength(), VP.elementWidth, VP.scaleFactor);
		painter.addRoad(l,t);
	}
	...
	private static class MyPainter implements SwingAnimatorPainter {
		...
		public void paint(Graphics g) {
			...
			for (Element<Light> e : lightElements) {
				if (e.x.getState().equals("green")) {
					g.setColor(Color.GREEN);
				} else if(e.x.getState().equals("red")){
				...
				XGraphics.fillOval(g, e.t, 0, 0, Property.getProperty().getCarLength(), VP.elementWidth);
			}
			g.setColor(Color.BLACK);
			for (Element<Road> e : roadElements) {
				XGraphics.fillRect(g, e.t, 0, 0, Property.getProperty().getRoadLength(), VP.elementWidth);
			}
			...
			for (Element<Road> e : roadElements) {
				...
				for (Car d : e.x.getCars().toArray(new Car[0])) {
					...
					XGraphics.fillOval(g, e.t, d.getPosition(), 0, Property.getProperty().getCarLength(), VP.elementWidth);
				}
			}
		}
	}
